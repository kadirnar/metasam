...
from metasam.sam2.utils.amg import (
    MaskData,
    area_from_rle,
    batch_iterator,
    batched_mask_to_box,
...
class SAM2AutomaticMaskGenerator:
    ...
    def __init__(
        ...
        """
        Using a SAM 2 model, generates masks for the entire image. Generates a grid of point prompts over the
        image, then filters low quality and duplicate masks. The default settings are chosen for SAM 2 with a
        HieraL backbone.
    ...
    def postprocess_small_regions(mask_data: MaskData, min_area: int, nms_thresh: float) -> MaskData:
        """
        Removes small disconnected regions and holes in masks, then reruns box NMS to remove any new
        duplicates.
    ...
    def refine_with_m2m(self, points, point_labels, low_res_masks, points_per_batch):
        ...
        for cur_points, cur_point_labels, low_res_mask in batch_iterator(points_per_batch, points,
                                                                         point_labels, low_res_masks):
